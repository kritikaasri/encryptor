...
if os.path.isfile('data.csv.enc'):
            ...
            if choice == 1:
                ...
                if df.shape[0]>2:
                    if df.loc[(df['FileName']=='all')].shape[0]>0:
                        ...
                        print("Multiple different encrypted files exist. Decrypt all files individually use this.")
                else:
    ...
    while True:
        clear()
        password = str(input("Setting up stuff. Enter a password that will be used for entering the decryption range: "))
